...
class Store {
  ...
  _getBaseAssets = async () => {
    try {
      ...
      baseAssets = baseAssets.filter((token) => {
        return BLACK_LIST_TOKENS.indexOf(token.id?.toLowerCase()) === -1;
      });
      let dupAssets = [];
  ...
  createPairStake = async (payload) => {
    try {
      ...
      if (token0.address === "WDOGE") {
        toki0 = CONTRACTS.WFTM_ADDRESS;
      }
      if (token1.address === "WDOGE") {
    ...
    try {
      ...
      this.emitter.emit(ACTIONS.TX_ADDED, {
        ...
        transactions: [
          ...
          {
            uuid: depositTXID,
            description: `Migrating liquidity pool from ${migrator.label} to Pepe Pool`,
            status: "WAITING",
          },
        ],
      });
  ...
  quoteSwap = async (payload) => {
    try {
      ...
      if (fromAsset.address === "WDOGE") {
        addy0 = CONTRACTS.WFTM_ADDRESS;
      }
      if (toAsset.address === "WDOGE") {
        addy1 = CONTRACTS.WFTM_ADDRESS;
      }
